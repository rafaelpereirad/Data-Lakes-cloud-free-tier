...
|----------|----------|----------| ----------|
| AMD Compute Instance | Compute | AMD based Compute VMs with 1/8 OCPU and 1 GB memory each | 2 AMD based compute VMs |
| Arm Compute Instance | Compute | Arm-based Ampere A1 cores and 24 GB of memory usable as 1 VM or up to 4 VMs | 3,000 OCPU hours and 18,000 GB hours per month |
| APEX | Developer Services | Build, integrate, and innovate with the industry-favorite low-code application platform | Up to 744 hours per instance |
| Flexible Network Load Balancer | Networking | Provides automated traffic distribution from one entry point to multiple servers in a backend set. | 1 instance |
| Load Balancer | Networking | Allows you to create highly available load balancers within your VCN with provisioned bandwidth. | 1 instance, 10 Mbps |
| Outbound Data Transfer | Networking | Outbound data transfer service | Up to 10 TB per month |
| Service Connector Hub | Networking | Cloud message bus platform for moving data between Oracle Cloud Infrastructure services. | 2 service connectors |
| Site-to-Site VPN | Networking | Provides a site-to-site IPSec connection between your on-premises network and your virtual cloud network (VCN). | 50 IPSec connections |
| VCN Flow Logs | Networking | VCN flow logs for auditing traffic and troubleshooting security lists. | Up to 10 GB per month shared across OCI Logging services |
| Virtual Cloud Networks (VCN) | Networking | Software-defined network that you set up in the Oracle Cloud Infrastructure data centers in a particular region. A subnet is a subdivision of a VCN. | 2 VCNs, includes IPv4 and IPv6 support |
| Application Performance Monitoring | Observability and Management | Provides a comprehensive set of features to monitor applications and diagnose performance issues. | Up to 1000 tracing events and 10 Synthetic Monitoring runs per hour |
| Email Delivery | Observability and Management | Fast and reliable managed solution for sending secured, high-volume marketing and transactional emails. | Up to 100 emails sent per day |
| Logging | Observability and Management | Highly scalable and fully managed single pane of glass for all the logs in your tenancy. | Up to 10 GB per month |
| Monitoring | Observability and Management | Use the service to query metrics and manage alarms for cloud resources' health, capacity, and performance. | Up to 500 million ingestion datapoints, 1 billion retrieval datapoints |
| Notification | Observability and Management | Allows alerts via alarms, event rules, and service connectors. Supported endpoints include email and SMS. | Up to 1 million sent through https per month, 1000 sent through email per month |
| Autonomous Database | Oracle Databases | Oracle Autonomous Transaction Processing, Autonomous Data Warehouse, Autonomous JSON Database, or APEX Application Development. | Up to two databases total |
| HeatWave | Oracle Databases | Automated AI/ML for transactions and lakehouse-scale analytics, including storage and backup. | One standalone HeatWave instance, 50 GB storage, 50 GB backup storage |
| NoSQL Database | Oracle Databases | Fully managed database cloud service with low latency, dynamic scalability, and reliable data storage for various data types. | Up to 133 million reads, 133 million writes, 25 GB storage per table, up to 3 tables |
| Console Dashboards | Others | Allows you to create custom dashboards in the OCI Console to monitor resources, diagnostics, and key metrics for your tenancy. | Up to 100 dashboards |
| Bastions | Security | Provides restricted and time-limited Secure Shell Protocol (SSH) access to target resources without public endpoints. | Up to 5 OCI Bastions |
| Certificates | Security | Provides certificate issuance, storage, and management capabilities, including revocation and automatic renewal. | Up to 5 Private CA and 150 private TLS certificates |
| Vault | Security | Stores and manages master encryption keys and secrets for secure access to resources. | Up to 20 key versions of master encryption keys protected by HSM, 150 Vault secrets |
| Archive Storage | Storage | Unstructured storage for archived data. | Up to 20GB total for standard, infrequent, and archive tiers |
| Block Volume Storage | Storage | Boot and block volume storage. | Up to 2 block volumes, 200 GB total, plus 5 volume backups |
| Object Storage | Storage | API requests for object storage services. | Up to 50,000 Object Storage API requests per month |
| Object Storage - Infrequent Access | Storage | Unstructured storage with infrequent access. | Up to 20GB total for standard, infrequent, and archive tiers |
| Object Storage - Standard | Storage | Unstructured storage with standard access. | Up to 20GB total for standard, infrequent, and archive tiers |

Oracle Cloud Free Tier. Available at: https://www.oracle.com/cloud/free/ (Accessed: 7 October 2024).

# Amazon Web Services (AWS)

| Service | Type | Description | Free |
|----------|----------|----------| ----------|
| Amazon DynamoDB | Database | Serverless, NoSQL, fully managed database with single-digit millisecond performance at any scale. | 25 GB of Storage. 25 provisioned Write Capacity Units (WCU). 25 provisioned Read Capacity Units (RCU). Enough to handle up to 200M requests per month. |
| AWS Lambda | Compute | Run code without thinking about servers or clusters. | 1,000,000 free requests per month. Up to 400,000 GB-seconds or 3.2 million seconds of compute time per month. |
| Amazon CodeWhisperer | Artificial Intelligence | Your AI-powered productivity tool for the IDE and command line. | Unlimited code suggestions. Includes Amazon Q chat in the IDE (Preview). 50 security scans per user per month. Reference tracking. Command line integration. Authenticate with AWS Builder ID. |
| Amazon SNS | Mobile | Fast, flexible, fully managed push messaging service. | 1,000,000 Publishes. 100,000 HTTP/S Deliveries. 1,000 Email Deliveries. |
| Amazon CloudWatch | Developer Tools | Monitoring for AWS cloud resources and applications. | 10 Custom Metrics and 10 Alarms. 1,000,000 API Requests. 5GB of Log Data Ingestion and Archive. 3 Dashboards with up to 50 Metrics Each per Month. |
| Amazon CloudFront | Content Delivery | Web service to distribute content to end users with low latency and high transfer speeds. | 1 TB of data transfer out to the internet per month. 10,000,000 HTTP/HTTPS Requests. 2,000,000 CloudFront Function invocations. |
| Amazon Cognito | Security, Identity, & Compliance | Simple and Secure User Sign-Up, Sign-In, and Access Control. | 50,000 MAUs per month. 10 GB of cloud sync storage for 12 months. 1,000,000 sync operations per month for 12 months. |
| Amazon SQS | Application Integration | Scalable queue for storing messages as they travel between computers. | 1,000,000 Requests. |
| Amazon SWF | Application Integration | Task coordination and state management service for Cloud applications. | 10,000 Activity Tasks. 30,000 Workflow-Days. 1,000 Initiated Executions. |
| AWS CodeBuild | Developer Tools | Fully managed build service that builds and tests code in the cloud. | 100 build minutes per month of build.general1.small compute type usage |
| AWS CodeCommit | Developer Tools | Highly scalable, managed source control service. | 5 active users per month. 50 GB-month of storage. 10,000 Git requests per month. |
| AWS CodePipeline | Developer Tools | Continuous delivery service for fast and reliable application updates. | 1 Active Pipeline per month |
| AWS Database Migration Service | Database | Migrate databases with minimal downtime. | 750 Hours of Amazon DMS Single-AZ dms.t2.micro instance usage. 50 GB of General Purpose (SSD) storage. |
| AWS Glue | Analytics | Simple, flexible, and cost-effective extract, transform, and load (ETL) service. | 1 Million objects stored in the AWS Glue Data Catalog. 1 Million requests per month to the AWS Glue Data Catalog. |
| AWS Key Management Service | Security, Identity, & Compliance | Managed service that provides easy encryption with administrative controls. | 20,000 free requests per month |
| AWS License Manager | Management Tools | Set rules to manage, discover, and report third-party license usage proactively. | Unlimited - set as many rules as you want to manage third-party license usage. |
| AWS Step Functions | Application Integration | Coordinate components of distributed applications. | 4,000 state transitions per month |
| AWS Storage Gateway | Storage | Hybrid cloud storage with seamless local integration and optimized data transfer. | First 100 GB per account free. No transfer charges into AWS. |
| AWS Well-Architected Tool | Management Tools | Review your architecture and adopt best practices using the AWS Well-Architected Tool. | Unlimited workload reviews |
| AWS X-Ray | Developer Tools | Analyze and debug your applications. | 100,000 traces recorded per month. 1,000,000 traces scanned or retrieved per month |
| AWS Security Hub | Security, Identity, & Compliance | Automate AWS security checks and centralize security alerts. | Free for 30 days. Afterward, 10,000 ingested findings per account per region per month |
| AWS CodeArtifact | Developer Tools | Fully managed artifact repository service for securely storing and sharing software packages. | 2 GB of storage per month. First 100,000 requests per month |
| AWS Migration Hub | Migration | Track the progress of application migrations and obtain EC2 recommendations. | Unlimited applications tracked and EC2 recommendations |
| AWS Application Discovery Service | Migration | Collects server specification information and performance data. | 1,000 applications per account |
| AWS Service Catalog | Management Tools | Create and manage catalogs of IT services approved for use on AWS. | 1,000 API calls per month |
| AWS CloudTrail | Management Tools | Log, monitor, and retain account activity across your AWS infrastructure. | 1 trail that delivers a single copy of management events in each region |
| AWS Systems Manager | Management Tools | Centralize operational data and automate tasks across AWS resources. | Free for 12 Systems Manager features, limits may apply |
| AWS CloudFormation | Management Tools | Model and provision cloud infrastructure resources with code for configuration compliance and faster troubleshooting. | 1,000 handler operations per month per account. $.0009 per handler operation over 1,000. |
| AWS Control Tower | Management Tools | The easiest way to set up and govern a secure multi-account AWS environment. | Always free with no additional charges. AWS services configured for your landing zone will incur costs based on usage. |
| AWS Organizations | Management Tools | Centrally manage billing, control access, and share resources across your AWS accounts. | Available at no additional charge to all AWS customers. |
...
| App Configuration | Developer Tools | Store and manage configurations for all your Azure apps. | 1,000 requests per day with 10 MB storage |
...
| Notification Hubs | Mobile | Send push notifications to any platform from any back end. | 1 million push notifications with free namespace |
